...
var $cssSelecToDay="selectedtoday"; // td tag: the selected day
...
if (!empty($_REQUEST['d'])) {
	$lad_d = split_date ($_REQUEST['d']);
	$this->lad_day = $this->mkActiveTime(0,0,1,$lad_d['month'], $lad_d['day'], $lad_d['year']);
}

if (!empty($_REQUEST['w'])) {
	$lad_wk = split_date ($_REQUEST['w']);
	$this->lad_wk = $this->mkActiveTime(0,0,1,$lad_wk['month'], $lad_wk['day'], $lad_wk['year']);
	$lad_wk_end = split_date (date('Y-m-d', strtotime('+6 day' , strtotime("{$_REQUEST['w']} 00:00:01 GMT"))));
	$this->lad_wk_end = $this->mkActiveTime(0,0,1,$lad_wk_end['month'], $lad_wk_end['day'], $lad_wk_end['year']);
}

if (!isset($_REQUEST['d']) && !isset($_REQUEST['w'])) {		// highlight todays date, if none are highlighted
	$this->lad_day = $this->mkActiveTime(0,0,1,date("m"), date("d"), date("Y"));
}
...
/*
********************************************************************************
PUBLIC addURL() -> changes the default url - rob curle
********************************************************************************
*/
function add_url($link=false,$javaScript=false){
...
return "<table class=\"".$this->cssMonthTable."\" cellpadding=\"2\" cellspacing=\"1\">\n";
}
/*
********************************************************************************
...
function mkMonthTitle(){
	if (!$this->monthNav){
		...
		$out.="<td class=\"".$this->cssMonthTitle."\" colspan=\"4\">";
...
function mkWeekDays(){
	if ($this->startOnSun){
		$out="<tr class=\"".$this->cssWeekDay."\"><td></td>";
...
function mkMonthBody(){
	$wk = date('Y-m-d', strtotime('-'.$this->firstday.' day' , strtotime($this->actyear."-".$this->actmonth."-01 00:01:00 GMT")));
	$out="<tr><td class=\"".$this->cssWeekDay."\">".$this->mkWkUrl($wk)."</td>";
	...
	for ($x=0; $x<=6; $x++){
		...
		if ($stop==1) break;
		$out.="<tr><td class=\"".$this->cssWeekDay."\">".$this->mkWkUrl($this->actyear."-".sprintf("%02u", $this->actmonth)."-".sprintf("%02u", $goon))."</td>";
...
function mkDay($var){
	$selected = false;
	$lad_day = $this->mkActiveTime(0,0,1,$this->actmonth, $var, $this->actyear);
	
	if (isset($this->lad_wk)) {
		if ($lad_day >= $this->lad_wk && $lad_day <= $this->lad_wk_end) {
			$selected = true;
		}
	}
	if (isset($this->lad_day)) {
		if ($lad_day == $this->lad_day) {
			$selected = true;
		}
	}	
	...
	if ($this->isEvent($var)){
		/* make event have a day link - Rob Curle 17 June 2005
		if ($this->eventUrl){
		...
		*/
		if ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday && $selected) $out="<td class=\"".$this->cssSelecToDay."\">".$linkstr.$eventContent."</td>";
		else if ($selected) $out="<td class=\"".$this->cssSelecDay."\">".$linkstr.$eventContent."</td>";
	...
	elseif ($var==$this->selectedday && $this->actmonth==$this->selectedmonth && $this->actyear==$this->selectedyear){
		if ($selected) $out="<td class=\"".$this->cssSelecDay."\">".$var.$eventContent."</td>";
		else if (!$this->dayLinks) $out="<td class=\"".$this->cssSelecDay."\">".$var.$eventContent."</td>";
	...
	elseif ($var==$this->daytoday && $this->actmonth==$this->monthtoday && $this->actyear==$this->yeartoday){
		if ($selected) $out="<td class=\"".$this->cssSelecToDay."\">".$var.$eventContent."</td>";
		else if (!$this->dayLinks) $out="<td class=\"".$this->cssToday."\">".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->cssToday."\">".$linkstr.$eventContent."</td>";
	}
	elseif ($this->getWeekday($var)==0 && $this->crSunClass){
		if ($selected) $out="<td class=\"".$this->cssSelecDay."\">".$var.$eventContent."</td>";
		else if (!$this->dayLinks) $out="<td class=\"".$this->cssSunday."\">".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->cssSunday."\">".$linkstr.$eventContent."</td>";
		
	}
	elseif ($this->getWeekday($var)==6 && $this->crSatClass){
		if ($selected) $out="<td class=\"".$this->cssSelecDay."\">".$var.$eventContent."</td>";
		else if (!$this->dayLinks) $out="<td class=\"".$this->cssSaturday."\">".$var.$eventContent."</td>";
		else $out="<td class=\"".$this->cssSaturday."\">".$linkstr.$eventContent."</td>";
	}
	else{
		if ($selected) $out="<td class=\"".$this->cssSelecDay."\">".$var.$eventContent."</td>";
		else if (!$this->dayLinks) $out="<td class=\"".$this->cssMonthDay."\">".$var.$eventContent."</td>";
...
function mkUrl($year,$month=false,$day=false){
	...
	return "<a href=\"".$this->url.$glue."d=".$year."-".sprintf("%02u", $month)."-".sprintf("%02u", $day)."\">".$day."</a>";
}

function mkWkUrl($week) {
	if (strpos($this->url,"?")) $glue="&amp;";
	else $glue="?";
	return "<a href=\"".$this->url.$glue."w={$week}\">wk</a>";
}
/*
********************************************************************************
